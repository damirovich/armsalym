...
public partial class RegistPage : ContentPage
{
    ...
    private async void OnButtonSendClicked(object sender, EventArgs e)
    {
        ...
        if (result == "OK")
        {
            await Application.Current.MainPage.DisplayAlert("Успешно", "Заявка на регистрацию отправлена. Подойдите в офис Компании Салым Финанс для получения логина и пароля", "OK");

        }
        else if (result == "not found")
        {
            await Application.Current.MainPage.DisplayAlert("Ошибка", "Неправильный номер телефона. Подойдите в офис Компании Салым Финанс для получения дополнительной информации", "OK");
